/**
 * @license Highcharts JS v3.0.1 (2012-11-02) (c) 20013-2014 Author: Gert
 *          Vaartjes License: www.highcharts.com/license version: 2.0.1
 */

/* jslint white: true */
/*
 * global window, require, phantom, console, $, document, Image, Highcharts,
 * clearTimeout, clearInterval, options, cb, globalOptions, dataOptions,
 * customCode
 */
...
(function(){
	...
	var config= {
		/*
		 * define locations of mandatory javascript files. Depending on
		 * purchased license change the HIGHCHARTS property to highcharts.js or
		 * highstock.js
		 */
		files:{
		...
		TIMEOUT:5000
	/* 5 seconds timout for loading images */
	}, mapCLArguments, render, startServer= false, args, pick, SVG_DOCTYPE= '<?xml version=\"1.0" standalone=\"no\"?><!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" \"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">', dpiCorrection= 1.4, system= require('system'), fs= require('fs'), serverMode= false;
	...
	render= function(params, exitCallback){
		...
		var page= require('webpage').create(), messages= {}, scaleAndClipPage, loadChart, createChart, input, constr, callback, width, output, outType, timer, renderSVG, convert, exit, interval, counter, imagesLoaded= false;
		...
		page.onConsoleMessage= function(msg){
			...
			/*
			 * Ugly hack, but only way to get messages out of the
			 * 'page.evaluate()' sandbox. If any, please contribute with
		...
		scaleAndClipPage= function(svg){
			...
			var zoom= 1, pageWidth= pick(params.width, svg.width), clipwidth, clipheight;
			 ...
			 * precedence : page.zoomFactor = params.scale ? zoom * params.scale :
			 * zoom;
			 */

			/*
			 * params.width has a higher precedence over scaling, to not break
			 ...
			 */
			if(outType === 'pdf'){
				// changed to a multiplication with 1.333 to correct systems dpi
				// setting
		...
		function loadImages(imgUrls){
			...
			for(i= 0;i < imgUrls.length;i+= 1){
				img= new Image();
				/*
				 * onload decrements the counter, also when error (perhaps 404),
				 * don't wait for this image to be loaded
				 */
		...
		createChart= function(constr, input, globalOptionsArg, dataOptionsArg, customCodeArg, outputType, callback, messages){
			...
			// Notice we don't use commandline parameter width here. Commandline
			// parameter width is used for scaling.
			...
			/*
			 * remove stroke-opacity paths, used by mouse-trackers, they turn up
			 * as as fully opaque in the PDF
			 */
			nodes= document.querySelectorAll('*[stroke-opacity]');
		...
		if(params.length < 1){
			...
			page.open('about:blank', function(status){
				...
				if(input.substring(0, 4).toLowerCase() === "<svg" || input.substring(0, 5).toLowerCase() === "<?xml" || input.substring(0, 9).toLowerCase() === "<!doctype"){
					...
					// We have a js file, let highcharts create the chart first
					// and grab the svg
	...
	startServer= function(host, port){
		...
		server.listen(host + ':' + port, function(request, response){
			...
			var jsonStr= request.postRaw || request.post, params, msg;
