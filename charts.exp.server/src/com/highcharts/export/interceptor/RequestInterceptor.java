...
public class RequestInterceptor extends HandlerInterceptorAdapter {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
		...
		if (httpStatus == 500) {
			monitor.addError();
			logger.log(Level.INFO, "Time={0} :: Time taken(ms) {1}{3} :: RequestMethod {5} :: Status {6} :: Referer={2}{3} :: Request parameters {4}", new Object[]{new Date().toString(), // 0
			...
			logger.log(Level.INFO, "Time={0} :: Time taken(ms) {1}{3} :: RequestMethod {4} :: Status {5} :: Referer={2}", new Object[]{new Date().toString(), // 0
			System.currentTimeMillis() - startTime, // 1
			request.getHeader("referer"), // 2
			lineSeparator, // 3
			request.getMethod(), // 4
