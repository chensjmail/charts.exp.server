...
public class ExportController extends HttpServlet {
	...
	public HttpEntity<byte[]> exporter(@RequestParam(value= "svg", required= false) String svg, @RequestParam(value= "type", required= false) String type, @RequestParam(value= "filename", required= false) String filename, @RequestParam(value= "width", required= false) String width, @RequestParam(value= "scale", required= false) String scale, @RequestParam(value= "options", required= false) String options, @RequestParam(value= "globaloptions", required= false) String globalOptions, @RequestParam(value= "constr", required= false) String constructor, @RequestParam(value= "callback", required= false) String callback, @RequestParam(value= "callbackHC", required= false) String callbackHC, @RequestParam(value= "async", required= false, defaultValue= "false") Boolean async, @RequestParam(value= "jsonp", required= false, defaultValue= "false") Boolean jsonp, HttpServletRequest request, HttpSession session) throws ServletException, InterruptedException, SVGConverterException, NoSuchElementException, PoolException,
		...
		/*
		 * Most JSONP implementations use the 'callback' request parameter and this overwrites the original callback parameter for chart creation with Highcharts. If JSONP is used we recommend using the requestparameter callbackHC as the callback for Highcharts. store the callback method name and reset the callback parameter, otherwise it will be used when creation charts
		 */
		if (jsonp) {
			async= true;
		...
		if (async) {
			...
			headers.add("Content-Disposition", "attachment; filename=" + filename.replace(" ", "_") + "." + mime.name().toLowerCase());
		}
	...
	public HttpEntity<byte[]> exportFromJson(@PathVariable("name") String name, @PathVariable("ext") String extension, @RequestBody String requestBody) throws SVGConverterException, TimeoutException, NoSuchElementException, PoolException {
	...
	private String convert(String svg, MimeType mime, String width, String scale, String options, String constructor, String callback, String globalOptions, String filename) throws SVGConverterException, PoolException, NoSuchElementException, TimeoutException, ServletException {
		...
		if (options != null) {
			...
			if (svg == null) {
				...
				throw new ServletException("The mandatory 'svg' or 'options' POST parameter is undefined.");
			} else {
				convertSvg= true;
				input= svg;
			}
	...
	public ModelAndView handleSVGRasterizeException(Exception ex, HttpServletResponse response) {
		ModelAndView modelAndView= new ModelAndView();
		modelAndView.setViewName("error");
		modelAndView.addObject("message", "Something went wrong while converting. " + StringEscapeUtils.escapeHtml(ex.getMessage()));
		response.setStatus(500);
		return modelAndView;
	}
